...
func TradingCommand() run.Commander {
	return run.Simple("trading", "calculate trading gain/loss", &Trading{})
}
...
func (t *Trading) Run(ctx context.Context, stdin io.Reader, stdout, stderr io.Writer, fsys fs.FS) error {
	...
	for _, tr := range transactions {
		...
		if strings.HasSuffix(row.Action, "buy") {
			...
			if st.ticker == "NVDA" {
				fmt.Fprintln(stdout, shares, total, st.shares, st.total)
			}
	...
	for key, val := range stocks {
		fmt.Fprintln(stdout, key, val)
	}

	fmt.Fprintln(stdout)
	fmt.Fprintln(stdout, gbp2gbx)
	fmt.Fprintln(stdout, gbp2usd)
	...
	for curr, res := range results {
		...
		fmt.Fprintln(stdout, curr, res, resGBP)
	}
