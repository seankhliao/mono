	...
	spec: {
		...
		"""
							...
							cors: {
								...
								properties: {
									allowCredentials: {
										description: "AllowCredentials indicates whether a request can include user credentials like cookies, authentication headers, or TLS client certificates."
								...
								properties: {
									grpc: {
										...
										properties: backendRef: {
											description: "BackendRef references a Kubernetes object that represents the backend server to which the authorization request will be sent. Only service Kind is supported for now."
		...
		"""
								...
								"x-kubernetes-validations": [{
									...
									rule: "has(self.grpc) ? (!has(self.grpc.backendRef.group) || self.grpc.backendRef.group == \"\") : true"
								}, {
									message: "kind is invalid, only Service (specified by omitting the kind field or setting it to 'Service') is supported"

									rule: "has(self.grpc) ? (!has(self.grpc.backendRef.kind) || self.grpc.backendRef.kind == 'Service') : true"
								}, {
									message: "group is invalid, only the core API group (specified by omitting the group field or setting it to an empty string) is supported"

									rule: "has(self.http) ? (!has(self.http.backendRef.group) || self.http.backendRef.group == \"\") : true"
								}, {
									message: "kind is invalid, only Service (specified by omitting the kind field or setting it to 'Service') is supported"

									rule: "has(self.http) ? (!has(self.http.backendRef.kind) || self.http.backendRef.kind == 'Service') : true"
								}]
							}
		...
		"""
									...
									logoutPath: {
										description: "The path to log a user out, clearing their credential cookies. If not specified, uses a default logout path \"/logout\""
							...
							targetRef: {
								description: "TargetRef is the name of the Gateway resource this policy is being attached to. This Policy and the TargetRef MUST be in the same namespace for this Policy to have effect and be applied to the Gateway."
